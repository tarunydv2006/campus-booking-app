...
const AdminDashboard = () => {
  ...
  if (!stats) return <section className="page"><div className="glass h-80 animate-pulse rounded-lg" /></section>;
        ...
        <div className="glass rounded-lg p-6">
          ...
          <div className="mt-5 space-y-3">
            {stats.mostBookedResources.length ? stats.mostBookedResources.map((item) => (
              <div key={item._id} className="flex items-center justify-between rounded-lg bg-white/60 p-3 dark:bg-white/5">
                <div>
          ...
          <div className="mt-5 flex h-64 items-end gap-2">
            {stats.trend.length ? stats.trend.map((item) => (
              <div key={item._id} className="flex flex-1 flex-col items-center gap-2">
                <div className="w-full rounded-t-lg bg-teal-500" style={{ height: `${Math.max(18, item.count * 28)}px` }} />
